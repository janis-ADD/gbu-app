-- ─── 0012_release_snapshot_with_review_intervals ─────────────────────────
-- Compliance-Doktrin: Snapshot enthält ALLE für die Anzeige relevanten
-- Felder zur Release-Zeit. Mit Migration 0011 sind neue Felder dazugekommen
-- (review_interval_months, review_trigger_events) — diese müssen ebenfalls
-- eingefroren werden, damit eine spätere UI-Änderung den Snapshot nicht
-- unbeabsichtigt verändert.
--
-- Reine Refactor-Migration der ra_release_gbu()-Funktion: kein Schema-Change.
-- ────────────────────────────────────────────────────────────────────────
...
begin
  ...
  -- NEU in 0012: review_interval_months + review_trigger_events
  select jsonb_build_object(
    ...
    'gbu',             jsonb_build_object(
                          ...
                          'review_trigger_events',  g.review_trigger_events
                       ),
